...
def load_class_labels(dataset_name="ILSVRC/imagenet-1k"):
    """
    Load ImageNet-1k class labels directly from Hugging Face dataset metadata.
    Returns a list of 1000 class names in order.
    """
    try:
        ds = load_dataset(dataset_name, split="train", streaming=True)
        label_names = ds.features["label"].names
        print(f"✅ Loaded {len(label_names)} ImageNet-1k class labels from Hugging Face")
        return label_names
    except Exception as e:
        print(f"⚠️ Could not load labels from Hugging Face: {e}")
